 ...
 */
class AwsConnectionParams extends pip_services3_commons_nodex_1.ConfigParams {
    /**
    ...
    validate(correlationId) {
        let arn = this.getArn();
        if (arn == "arn:aws::::") {
            throw new pip_services3_commons_nodex_3.ConfigException(correlationId, "NO_AWS_CONNECTION", "AWS connection is not set");
        }
        if (this.getAccessId() == null) {
            throw new pip_services3_commons_nodex_3.ConfigException(correlationId, "NO_ACCESS_ID", "No access_id is configured in AWS credential");
        }
